(function ($) {
    ...
    var $toggle      = $('#any2html_enabled');
    var $toggleLabel = $('#any2html-toggle-label');
    ...
    /* ── Lock / unlock toggle + save button ──────────────── */
    function setToggleLock(lock) {
        if (lock) {
            $toggle.prop('checked', false).prop('disabled', true);
            $toggleLabel.addClass('any2html-toggle-locked');
            if ( ! $('.any2html-lock-notice').length) {
                $toggleLabel.closest('.any2html-card-body').append(
                    ...
                    ' A valid API key is required to enable this feature.' +
                    '</div>'
                );
            }
            $('#any2html-form-footer').hide();
        } else {
            $toggle.prop('disabled', false);
            $toggleLabel.removeClass('any2html-toggle-locked');
            $('.any2html-lock-notice').remove();
    ...
    /* ── Block toggle click if locked ────────────────────── */
    $toggleLabel.on('click', function (e) {
        if ($toggle.prop('disabled')) {
            e.preventDefault();
            $(this).addClass('any2html-toggle-shake');
            setTimeout(function () {
                $toggleLabel.removeClass('any2html-toggle-shake');
            }, 400);
        }
    });
